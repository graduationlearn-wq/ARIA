...
def import_template():
    """Download a sample CSV showing the columns the importer understands."""
    return PlainTextResponse(
        lead_importer.template_csv(),
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=beyondsure_lead_import_template.csv"},
    )
...
async def import_leads(
    ...
    """
    Import leads from an uploaded CSV/Excel file. Every created lead is owned by
    the uploader (the current user), so imported leads land under their pipeline.
    Returns a summary: imported / duplicates / skipped rows with reasons.
    """
    data = await file.read()
    if len(data) > MAX_IMPORT_BYTES:
        raise HTTPException(status_code=400, detail="File too large (max 5 MB)")
    try:
        rows = lead_importer.parse_rows(file.filename or "", data)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    if not rows:
        raise HTTPException(status_code=400, detail="No rows found in the file.")
    if len(rows) > lead_importer.MAX_ROWS:
        raise HTTPException(
            status_code=400,
